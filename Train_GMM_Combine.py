...
##  ClusterNum: int, number of clusters one wants to train
    ...
    for image in images:
        trainedInput = np.hstack((HandleImage_HSV(image,1), HandleImage_RGB(image, 1)))
    ...
    joblib.dump(model, './GMMmodel_Combine.pkl')
    print("Trained model successfully dumped to './GMMmodel_Combine.pkl'!")


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('--train_path', type=str, default=None) # pass in the path of the directory that contains all the train images
